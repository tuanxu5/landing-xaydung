...
import type { Recruitment, UpdateRecruitmentDto, ApiError } from '@/types';
...
export default function EditRecruitmentPage() {
  ...
  const [formData, setFormData] = useState<UpdateRecruitmentDto>({
    banner: '',
    title: '',
    quantity: 1,
    location: '',
  ...
  const fetchRecruitment = async () => {
    try {
      setLoading(true);
      const recruitment = await recruitmentsApi.getById(recruitmentId);
      ...
      setFormData({
        ...
        candidateRequirements: recruitment.candidateRequirements,
        salary: recruitment.salary,
        benefits: recruitment.benefits,
        contact: recruitment.contact,
        applicationDeadline: recruitment.applicationDeadline.split('T')[0],
        isActive: recruitment.isActive,
      });

      if (recruitment.banner) {
        setBannerPreview(recruitment.banner);
      }
    } catch (error) {
  ...
  const validateForm = (): boolean => {
    ...
    if ((formData.quantity || 0) < 1) newErrors.quantity = 'Số lượng phải lớn hơn 0';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const recruitmentData: UpdateRecruitmentDto = {
        ...formData,
        banner: bannerUrl,
      };

      await recruitmentsApi.update(recruitmentId, recruitmentData);
      snackbar.success('Tin tuyển dụng đã được cập nhật!');
      router.push('/admin/recruitments');
    } catch (error) {
      const err = error as ApiError;
      ...
      snackbar.error(err.message || 'Không thể cập nhật tin tuyển dụng');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Chỉnh sửa tin tuyển dụng</h1>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="flex flex-col gap-3">
              <Button
                ...
                className="w-full"
              >
                {saving ? 'Đang lưu...' : 'Cập nhật tin tuyển dụng'}
              </Button>
              <Button
                type="button"
                variant="outline"
